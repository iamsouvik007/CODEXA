# Lesson 04: Loop, Number, math and String
...
![Cheatsheet 4](/notes/4/cheetsheet4.png)

### Quick Summary

* **if/else**: Used to run different blocks of code based on conditions.
* **Loops**: `for` (when you know the exact number of repetitions), `while` (keep going until the condition is false), `do...while` (runs at least once).
* **Number**: All numbers are 64-bit floating point. Floating point inaccuracies can occur (e.g., `0.1 + 0.2`).
* **Math**: Use `Math.random()` with `Math.floor()` for random integers, `Math.round()`, `Math.max()`, etc.
* **String**: Text enclosed in quotes (\`'\`, \`"\`, or \`\`\`). Strings are immutable. Useful methods: \`.length\`, \`.toUpperCase()\`, \`.includes()\`, \`.replace()\`, \`.slice()\`, and \`.split()\`.
<!-- REVISION_END -->