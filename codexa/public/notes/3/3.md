# Lesson 03: Operator and Data type in JS
...
### **In-Depth Lesson Notes: JavaScript Operators**
...
![Cheatsheet 3](/notes/3/cheetsheet3.png)

### Quick Summary

* **Operators**: Assignment (`=`), Arithmetic (`+`, `-`, `*`, `/`, `%`, `**`), Comparison (`==`, `===`), Logical (`&&`, `||`, `!`).
* **Coercion**: JavaScript automatically converts types (e.g., `1 + "2" = "12"`).
* **Equality**: Loose equality `==` allows coercion; strict equality `===` checks both value and type. Always use `===`.
