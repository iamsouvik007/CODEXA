...
![Cheatsheet 2](/notes/2/Cheetsheet2.png)

### Quick Summary

* **Variables**: `let` (block-scoped, mutable), `const` (block-scoped, immutable reference), `var` (function-scoped, legacy).
* **Data Types**: Primitives include Number, String, Boolean, null, undefined, Symbol, BigInt. Non-primitives are Objects, Arrays, Functions.
* **Pass by Value vs Reference**: Primitives are copied by value; objects are passed by reference (changes affect the original).
* **Typeof**: Returns the type of a value. Note: `typeof null` is "object" due to a legacy bug.
<!-- REVISION_END -->